...
from utils.MY_GNN import collate_molgraphs, run_an_eval_epoch_heterogeneous2,EarlyStopping, run_an_eval_predmerge,run_an_eval_epoch_heterogeneous_return_weight_py,run_an_eval_epoch_pih,\
 set_random_seed, MGA
...
args['task_name'] = 'zhangnew'  # change
args['data_name'] = 'zhangnew' # change
args['times'] = 1

# selected task, generate select task index, task class, and classification_num
args['select_task_list'] = ['E', 'S','A', 'B']  # change
args['select_task_index'] = []
args['classification_num'] = 0
args['regression_num'] = 0
args['all_task_list'] = ['E', 'S','A', 'B']  # change
# generate select task index  生成选择任务索引
...
for time_id in range(args['times']):
    ...
    model = MGA(in_feats=args['in_feats'], rgcn_hidden_feats=args['rgcn_hidden_feats'],
                   n_tasks=4, rgcn_drop_out=args['rgcn_drop_out'],
    ...
    selected_data = pd.read_csv('interpretation/data for interpretation/ppLFER_for_interpretation.csv')
...
run_an_eval_predmerge(args, model, val_loader, output_path='result/'+args['task_name']+'val.csv')
run_an_eval_predmerge(args, model, train_loader, output_path='result/'+args['task_name']+'train.csv')
run_an_eval_predmerge(args, model, test_loader, output_path='result/'+args['task_name']+'test.csv')