...
def weight_visulize(smiles, atom_weight):
    print(smiles)
    atom_weight_list = atom_weight.squeeze().cpu().numpy().tolist()
...
def weight_visulize_py(smiles, atom_weight,tasks_index):
    print(smiles)
    atom_weight_list = atom_weight.squeeze().cpu().numpy().tolist()
    max_atom_weight_index = atom_weight_list.index(max(atom_weight_list))
    significant_weight = atom_weight[max_atom_weight_index]
    ...
    drawer.FinishDrawing()
    svg = drawer.GetDrawingText()
    with open(r'./mutiS1126/'+smiles_name+'.svg','w') as f:
        f.write(svg)
    #MolToFile(mol, r'./A/'+smiles_name+'.png', highlightAtoms=range(0, mol.GetNumAtoms()), highlightBonds=range(0, mol.GetNumBonds()), highlightAtomColors=atom_colors, highlightBondColors=bond_colors)

import csv

def weight_visulize_py2(smiles, atom_weight, tasks_index):
    print(smiles)
    atom_weight_list = atom_weight.squeeze().cpu().numpy().tolist()
    mol = Chem.MolFromSmiles(smiles)
    mol = Chem.AddHs(mol)
    ...
    bond_colors = {}
    for i in range(mol.GetNumAtoms()):
        atom_colors[i] = plt_colors.to_rgba(float(atom_weight_list[i]))
    for i in range(mol.GetNumBonds()):
        bond = mol.GetBondWithIdx(i)
        u = bond.GetBeginAtomIdx()
        v = bond.GetEndAtomIdx()
        x = atom_weight_list[u]
        y = atom_weight_list[v]
        bond_weight = (x+y)/2
        bond_colors[i] = plt_colors.to_rgba(float(abs(bond_weight)))
    rdDepictor.Compute2DCoords(mol)
    drawer = rdMolDraw2D.MolDraw2DSVG(700, 700)
    ...
    drawer.FinishDrawing()
    svg = drawer.GetDrawingText()
    with open(f'./mutiE1126/{smiles_name}.svg', 'w') as f:
        f.write(svg)
