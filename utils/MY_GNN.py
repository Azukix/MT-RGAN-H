...
import pandas as pd
from utils import weight_visualization2


class WeightAndSum(nn.Module):
    def __init__(self, in_feats, task_num=5, attention=True, return_weight=False):
...
class Meter(object):
    ...
    def rmse(self):
        ...
        for task in range(n_tasks):
            ...
            task_y_true = y_true[:, task][task_w != 0].numpy()
            task_y_pred = y_pred[:, task][task_w != 0].numpy()
            scores.append(np.sqrt(mean_squared_error(task_y_pred, task_y_true).item()))
        return scores
...
def run_an_eval_epoch_heterogeneous2(args, model, data_loader):
    model.eval()
    eval_meter_c = Meter()
    eval_meter_r = Meter()
    with torch.no_grad():
...
def run_an_eval_epoch_heterogeneous_return_weight_py(args, model, data_loader, vis_list=None, vis_task='A'):
    model.eval()
    with torch.no_grad():
        for batch_id, batch_data in enumerate(data_loader):
            smiles, bg, labels, mask = batch_data
                        ...
                        if args['all_task_list'][tasks_index] == vis_task:
                            if labels[mol_index, tasks_index]!=123456:
                                ...
                                if tasks_index < 0:
                                    ...
                                    print('Predict values:', logits[mol_index, tasks_index])
                                print('True values:', labels[mol_index, tasks_index])
                                weight_visualization2.weight_visulize_py2(atom_smiles, one_atom_weight,tasks_index)
                else:
                    continue
...
def run_an_eval_epoch_heterogeneous_return_weight_py2(args, model, data_loader, vis_list=None, vis_task='S'):
    model.eval()
    with torch.no_grad():
        for batch_id, batch_data in enumerate(data_loader):
            smiles, bg, labels, mask = batch_data
